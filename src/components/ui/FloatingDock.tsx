...
export const FloatingDock = ({
  ...
  if (position === 'bottom') {
    return <FloatingDockBar items={items} className={desktopClassName} />;
  }
  return (
    <>
...
type DockItem = { title: string; icon: React.ReactNode; href: string; onClick?: () => void; active?: boolean };
...
const FloatingDockDesktop = ({
  ...
  return (
    ...
    >
      {items.map((item) => (
        <IconContainer mouseX={mouseX} key={item.title} title={item.title} icon={item.icon} href={item.href} onClick={item.onClick} active={item.active} />
      ))}
    </motion.div>
  );
};
    ...
    <a href={href} onClick={(e) => { if (onClick) { e.preventDefault(); onClick(); } }}>
      <motion.div
        ...
        className={`relative flex aspect-square items-center justify-center rounded-full transition-colors ${
          active
            ? 'bg-brand-emerald dark:bg-brand-emerald'
